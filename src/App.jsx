...
export default function App() {
  ...
  const sections = useMemo(() => ['what', 'how', 'create', 'dice', 'rules', 'start'], [])
  ...
  // Scroll progress + to-top visibility
  useEffect(() => {
    const onScroll = () => {
      setTop(window.scrollY < 6)
  ...
  const onHeroMove = (e) => {
    ...
    const tx = x * 12
    const ty = y * 10
    heroParallaxRef.current.style.transform = `translate3d(${tx}px, ${ty}px, 0)`
  }
  ...
  const roll = () => {
    ...
    const tempAnim = () => {
      ...
      // wobble class for D20 during roll
      setResult({ temp: true, a: rollD20(), b: rollD20() })
      if (t < duration) {
        ...
        const b = rollD20()
        let used = a
        if (mode === 'adv') used = Math.max(a, b)
        if (mode === 'dis') used = Math.min(a, b)
        const total = used + Number(modifier || 0)
        const final = { a, b, used, total, mode, modifier: Number(modifier || 0) }
        setResult(final)
  ...
  return (
    <div className={`site ${simplify ? 'simplify' : ''}`}>
      <style>{`
        :root { --bg: #0b1020; --card: #121933; --muted: #9fb0d6; --text: #e6ecff; --accent: #e7a43a; --accent-2: #eb5a46; --ring: rgba(231,164,58,0.45); --shadow: 0 10px 30px rgba(0,0,0,.45), 0 0 40px rgba(231,164,58,.08); }
        * { box-sizing: border-box; }
        ...
        /* Top progress bar */
        .progressbar { position: fixed; top: 0; left: 0; height: 3px; width: 100%; z-index: 60; background: linear-gradient(90deg, rgba(231,164,58,.35) calc(var(--scroll) * 1%), transparent 0); pointer-events: none; }

        /* Header */
        header.site-header { position: sticky; top: 0; z-index: 50; backdrop-filter: blur(8px); background: linear-gradient(to bottom, rgba(11,16,32,.9), rgba(11,16,32,.45)); border-bottom: 1px solid rgba(231,164,58,.15); transition: box-shadow .2s ease; box-shadow: ${top ? 'none' : '0 6px 30px rgba(0,0,0,.25)'} }
        .nav { max-width: 1200px; margin: 0 auto; padding: .75rem 1rem; display: flex; align-items: center; justify-content: space-between; gap: .5rem; }
        ...
        .nav-links a { padding: .45rem .7rem; border-radius: 8px; border: 1px solid transparent; }
        .nav-links a.active { border-color: rgba(231,164,58,.35); background: rgba(231,164,58,.12); text-decoration: none; }
        ...
        .card { background: linear-gradient(180deg, rgba(255,255,255,.02), rgba(255,255,255,.01)); border: 1px solid rgba(231,164,58,.17); border-radius: 16px; padding: 18px; box-shadow: var(--shadow); position: relative; overflow: hidden; transform-style: preserve-3d; perspective: 800px; transition: transform .12s ease; }
        ...
        .d20 { width: clamp(160px, 40vw, 260px); height: clamp(160px, 40vw, 260px); background: radial-gradient(circle at 30% 30%, rgba(231,164,58,.35), rgba(231,164,58,.12)); border: 1px solid rgba(231,164,58,.35); clip-path: polygon(50% 0%, 93% 25%, 93% 75%, 50% 100%, 7% 75%, 7% 25%); display: grid; place-items: center; border-radius: 12px; box-shadow: 0 0 80px rgba(231,164,58,.12) inset, 0 30px 80px rgba(0,0,0,.45); position: relative; cursor: pointer; user-select: none; }
        .d20::after { content: '${rolling ? '' : '20'}'; font-weight: 800; font-size: clamp(2.5rem, 6vw, 4rem); color: var(--accent); filter: drop-shadow(0 0 12px rgba(231,164,58,.35)); }
        .d20.rolling { animation: wobble .6s ease-in-out infinite; }
        @keyframes wobble { 0% { transform: rotate(0) } 25% { transform: rotate(3deg) } 50% { transform: rotate(0) } 75% { transform: rotate(-3deg) } 100% { transform: rotate(0) } }
        ...
        .roller { display: grid; gap: 10px; place-items: center; margin-top: 16px; }
        .roller .controls { display: flex; gap: 8px; flex-wrap: wrap; align-items: center; justify-content: center; }
        .seg { display: inline-flex; align-items: center; gap: 8px; padding: 8px 10px; border-radius: 12px; border: 1px solid rgba(231,164,58,.22); background: rgba(231,164,58,.06); }
        .seg input[type="number"] { width: 64px; background: transparent; border: 1px solid rgba(231,164,58,.25); color: var(--text); border-radius: 8px; padding: 6px 8px; }
        ...
        /* Modal */
        .modal { position: fixed; inset: 0; display: grid; place-items: center; background: rgba(3,6,14,.6); backdrop-filter: blur(6px); z-index: 70; }
        ...
        /* Floating to-top */
        .to-top { position: fixed; right: 18px; bottom: 18px; z-index: 60; opacity: ${showToTop ? 1 : 0}; pointer-events: ${showToTop ? 'auto' : 'none'}; transition: opacity .2s ease; }
      `}</style>
          ...
          <div className="nav-links" role="navigation" aria-label="Primary">
            {sections.map((id) => (
              <a key={id} href={`#${id}`} className={active === id ? 'active' : ''} aria-current={active === id ? 'page' : undefined}>
                {id === 'what' ? 'What' : id.charAt(0).toUpperCase() + id.slice(1)}
              </a>
            ))}
          </div>
      ...
      <section className="hero" aria-label="Hero" onMouseMove={onHeroMove}>
        <div className="spline-wrap" aria-hidden>
          <Spline scene="https://prod.spline.design/atN3lqky4IzF-KEP/scene.splinecode" style={{ width: '100%', height: '100%' }} />
        </div>
        <div className="overlay" aria-hidden />
        <div className="content">
          <div className="hero-parallax" ref={heroParallaxRef}>
        ...
        <section id="what" aria-labelledby="what-title">
          <div className="container">
            ...
            <div className="cards reveal" role="list">
              {[
                { icon: 'dm', title: 'Dungeon Master', p1: 'The DM is the storyteller and director. They describe scenes, play the world’s characters and set challenges with fairness and fun in mind.', p2: 'They don’t compete against players—the goal is to make the adventure exciting, coherent and full of meaningful choices.' },
                { icon: 'players', title: 'Players', p1: 'You play the heroes. You explore, investigate and make decisions that shape what happens next.', p2: 'Your character has strengths and flaws. You’ll work as a team, lean on each other’s talents and grow through the journey.' },
                { icon: 'dice', title: 'Dice', p1: 'Dice add uncertainty and drama. A roll can open doors, spring traps or turn the tide of a battle.', p2: 'You’ll mostly roll a twenty-sided die (d20) and add a modifier. Higher is better—your chances improve with expertise or advantages.' }
              ].map((c, i) => (
                <article key={i} className="card tilt" role="listitem" onMouseMove={(e)=>{
                  const r=e.currentTarget.getBoundingClientRect();
                  const x=(e.clientX-r.left)/r.width-.5; const y=(e.clientY-r.top)/r.height-.5;
                  e.currentTarget.style.transform=`rotateX(${(-y*6).toFixed(2)}deg) rotateY(${(x*8).toFixed(2)}deg)`
                }} onMouseLeave={(e)=>{e.currentTarget.style.transform='rotateX(0) rotateY(0)'}}>
                  <div className="icon" aria-hidden><Icon name={c.icon} /></div>
                  <h3>{c.title}</h3>
                  <p>{c.p1}</p>
                  <p className="verbose">{c.p2}</p>
                </article>
              ))}
            </div>
          </div>
        ...
        <section id="how" aria-labelledby="how-title">
          <div className="container">
            ...
            <div className="steps reveal" role="list">
              <article className="card step tilt" role="listitem" onMouseMove={(e)=>{const r=e.currentTarget.getBoundingClientRect();const x=(e.clientX-r.left)/r.width-.5;const y=(e.clientY-r.top)/r.height-.5;e.currentTarget.style.transform=`rotateX(${(-y*6).toFixed(2)}deg) rotateY(${(x*8).toFixed(2)}deg)`}} onMouseLeave={(e)=>{e.currentTarget.style.transform='rotateX(0) rotateY(0)'}}>
        ...
        <section id="create" aria-labelledby="create-title">
          <div className="container">
            ...
            <div className="progress reveal" role="list">
              {[
                { t: 'Idea', d:'Who do you want to be? A brave knight, a clever rogue, a kindly healer or a curious wizard?', m:'Start with a vibe or a story hook—"exiled scholar seeking redemption" or "smith’s apprentice with a dragonmark." Personality leads the numbers.' },
                { t: 'Choose race and class', d:'Pick a heritage (like human, elf or dwarf) and a class (fighter, cleric, wizard and more). These shape your abilities.', m:'Race grants traits; class defines your role. New to the game? Fighter or cleric are straightforward and rewarding from level one.' },
                { t: 'Distribute ability scores', d:'Assign numbers to Strength, Dexterity, Constitution, Intelligence, Wisdom and Charisma.', m:'Use standard array (15, 14, 13, 12, 10, 8). Put your highest score where your class needs it most—for example, Strength for fighters.' },
                { t: 'Equipment and personality', d:'Choose starting gear and note ideals, bonds and flaws. These details bring your hero to life.', m:'Let gear reflect your story—engraved shield from a mentor, tattered journal of maps. Personality prompts guide roleplay without pressure.' },
              ].map((p,i)=> (
                <article key={i} className="card panel" role="listitem">
                  <h3>{p.t}</h3>
                  <p>{p.d}</p>
                  <footer>
            ...
            <aside className="info reveal" role="note">New here? You can start in minutes with prebuilt characters. Focus on playing; customize later as you learn.</aside>
          </div>
        </section>
        ...
        <section id="dice" aria-labelledby="dice-title">
          <div className="container">
            ...
            <div className="dice-area reveal" aria-label="Dice system graphic">
              <div className={`d20 ${rolling ? 'rolling' : ''}`} aria-label="d20" onClick={roll} title="Click to roll!" />
            ...
            <div className="roller">
              <div className="controls">
                <div className="seg" role="group" aria-label="Roll mode">
                  <label><input type="radio" name="mode" checked={mode==='normal'} onChange={()=>setMode('normal')} /> Normal</label>
                  <label><input type="radio" name="mode" checked={mode==='adv'} onChange={()=>setMode('adv')} /> Advantage</label>
                  <label><input type="radio" name="mode" checked={mode==='dis'} onChange={()=>setMode('dis')} /> Disadvantage</label>
                </div>
                <div className="seg" role="group" aria-label="Modifier">
                  <span>Modifier</span>
                  <input type="number" value={modifier} onChange={(e)=> setModifier(e.target.value)} />
                </div>
                <button className="btn primary" onClick={roll} disabled={rolling} aria-live="polite">{rolling ? 'Rolling…' : 'Roll d20'}</button>
              </div>
              {result && !result.temp && (
                <div className="scene" aria-live="polite">
                  <p><strong>Result:</strong> {result.total} <span className="verbose">(d20 {result.mode==='normal' ? '' : result.mode} used {result.used}{result.mode!=='normal' ? ` | other ${result.used===result.a ? result.b : result.a}` : ''} {result.modifier>=0?'+':''}{result.modifier})</span></p>
                  <p><em>{result.total >= 12 ? 'Success!' : 'Not quite—try a new approach.'}</em></p>
                </div>
              )}
              {history.length>0 && (
                <div className="scene" aria-label="Recent rolls">
                  <strong>Recent rolls:</strong>
                  <ul>
                    {history.map((h,i)=>(<li key={i}>[{h.mode}] {h.used}{h.modifier>=0?'+':''}{h.modifier} = {h.total}</li>))}
                  </ul>
                </div>
              )}
            </div>
            ...
            <div className="example reveal">
              ...
              <div className="scene" aria-label="Illustrated number example">
                ...
                <p>Roll d20 + Dexterity + proficiency. If your total meets or beats 12, the chest opens. If not, you can try another tactic—or face a noisy mishap.</p>
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="container">
            ...
            <div className="rules reveal" role="list">
              {[
                ...
                <article key={i} className="card tilt" role="listitem" onMouseMove={(e)=>{const r=e.currentTarget.getBoundingClientRect();const x=(e.clientX-r.left)/r.width-.5;const y=(e.clientY-r.top)/r.height-.5;e.currentTarget.style.transform=`rotateX(${(-y*6).toFixed(2)}deg) rotateY(${(x*8).toFixed(2)}deg)`}} onMouseLeave={(e)=>{e.currentTarget.style.transform='rotateX(0) rotateY(0)'}}>
                  <h3>{r.t}</h3>
                  <p>{r.d1}</p>
        ...
        <section id="start" aria-labelledby="start-title">
          <div className="container">
            ...
            <div style={{ marginTop: 16, display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
              <a className="btn primary" href="#" onClick={(e)=> e.preventDefault()} aria-label="Download the beginner guide"><Icon name="download" /> Download the beginner guide</a>
              <a className="btn" href="#dice" aria-label="Try the dice roller">Try the dice roller</a>
            </div>
          </div>
        </section>
      ...
      <a href="#top" className="to-top btn" onClick={(e)=>{e.preventDefault(); window.scrollTo({top:0, behavior:'smooth'})}} aria-label="Back to top">Top</a>
    </div>
  )
}
...
function Glossary(){
  ...
  const items = useMemo(()=>[
    { k:'Initiative', v:'Determines turn order in combat. Everyone rolls a d20 and adds their initiative modifier.'},
    { k:'Saving throw', v:'A reactive roll to resist danger—dodging a blast, shaking off poison, standing firm against fear.'},
    { k:'Advantage / Disadvantage', v:'Roll two d20s and keep the higher/lower to reflect favorable or risky circumstances.'},
    { k:'DC (Difficulty Class)', v:'The target number you must meet or beat to succeed at a task.'},
    { k:'Proficiency bonus', v:'A bonus you add to checks you’re trained in; it increases as you level up.'},
    { k:'Ability check', v:'Roll d20 + relevant ability + proficiency (if applicable) to attempt a non-combat task.'},
  ], [])
  const filtered = items.filter(i => (i.k + ' ' + i.v).toLowerCase().includes(q.toLowerCase()))
  return (
    <div style={{ display:'grid', gap:12, marginTop:12 }}>
      <input placeholder="Search terms…" value={q} onChange={(e)=>setQ(e.target.value)} style={{ width:'100%', background:'transparent', border:'1px solid rgba(231,164,58,.35)', color:'var(--text)', borderRadius:12, padding:'10px 12px' }} />
      <div style={{ display:'grid', gap:10 }}>
        {filtered.map((it,i)=> (
          <div key={i} className="card"><strong>{it.k}:</strong> {it.v}</div>
        ))}
        {filtered.length===0 && <div className="card">No matching terms.</div>}
      </div>
    </div>
  )
}
